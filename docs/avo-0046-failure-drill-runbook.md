...
variable match, exact synthetic commit/tree, exact check names and App 15368, freshness,
protection manifest, PR/base/head identities, provider receipts, and ref cleanup.
...
| 3. Check identity/freshness | Required check is missing, stale, incomplete, duplicated, attached to PR head only, or from the wrong App/SHA. Attester rejects. | Real `GitHubIntegrationProvider` parser against controlled transports; exact synthetic success plus head-only, wrong SHA/App, stale, incomplete, and duplicate-context rejection scenarios. | Base-controlled workflow run on exact validation SHA; provider check-run manifest must show both pinned names, App 15368, exact SHA, success, and freshness. Repeat at least one rejected identity condition without weakening the gate. |
...
* exact required check contexts, App IDs, status/conclusion, completion timestamps,
  and freshness decision;
...
Stop and preserve the state root on any stale base, retarget, missing/mismatched check,
workflow digest mismatch, protection drift, ambiguous provider mutation, unexpected
parent/tree, duplicate mutation, main change, cleanup ambiguity, or credential-boundary
failure. Recovery uses the same state root and durable identities; it does not mint a
new operation to hide ambiguity.
