...
- The Windows workspace is now the controlling repository and has a private authenticated remote,
...
and coverage baseline, controlling repository, private remote, baseline tag, and recovery rehearsal
now pass. Promotion remains blocked on AVO-004.2 solely because the current provider plan does not
permit server-side protection for this private repository.
